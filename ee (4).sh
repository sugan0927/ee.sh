...
[sshd]
enabled = true
# Use the systemd journal instead of a log file. Many VPS/container images
# (this one included) don't run rsyslog, so /var/log/auth.log never gets
# created and fail2ban's config test fails with "Have not found any log
# file for sshd jail". sshd's own logs always reach the journal regardless
# of whether rsyslog is installed, so backend=systemd works everywhere.
backend = systemd
...
EOF
    ...
    # Validate config before (re)starting. fail2ban is a security nice-to-
    # have, not core to the web server — on an environment we haven't seen
    # before, a jail could still fail for a reason we didn't anticipate
    # (missing log file, unavailable backend, etc). Rather than letting
    # `die` (and `set -e`) abort the ENTIRE LEMP install over fail2ban,
    # fall back step by step and, worst case, skip fail2ban with a warning
    # so nginx/PHP/MariaDB still finish installing.
    if ! fail2ban-client -t &>/tmp/fail2ban-test.log; then
        warn "fail2ban config test failed once — retrying with sshd only:"
...
EOF
        if ! fail2ban-client -t &>/tmp/fail2ban-test.log; then
            warn "fail2ban still failed its config test, see /tmp/fail2ban-test.log:"
            cat /tmp/fail2ban-test.log
            warn "Skipping fail2ban for now — this does NOT block the rest of the LEMP install."
            warn "You can retry later once the server is up: sudo systemctl restart fail2ban"
            return 0
        fi
    fi
...
EOF
    # Non-fatal: this unit just applies config and isn't essential to the
    # web server itself, so a restart hiccup here shouldn't abort install.
    systemctl restart unattended-upgrades 2>/dev/null || warn "Could not restart unattended-upgrades (non-critical, continuing)."
}
...
# Self-install to /usr/local/bin/ee on first real run, so the person can
# just type `ee help`, `ee create ...` etc. from anywhere afterwards,
# instead of only ever being able to run it as `./ee.sh` from wherever
# it happened to be downloaded. Silent, best-effort, never fatal.
self_install_cli() {
    local target="/usr/local/bin/ee"
    local self_path
    self_path=$(readlink -f "$0" 2>/dev/null || echo "$0")
    [[ "$self_path" == "$target" ]] && return 0
    if [[ ! -e "$target" ]] || ! cmp -s "$self_path" "$target" 2>/dev/null; then
        if cp "$self_path" "$target" 2>/dev/null && chmod +x "$target" 2>/dev/null; then
            info "Installed this script as $target — from now on you can just run: ee help"
        fi
    fi
}
self_install_cli
